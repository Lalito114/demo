...
public class claveFinalizaVenta extends BaseActivity implements FingerprintListener, View.OnClickListener  {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnhuella.setOnClickListener(claveFinalizaVenta.this);
    ...
    private void validaPosicionDisponible(final String posicioncarga, final String usuario){
        // URL para obtener los empleados  y huellas de la posición de carga X
        String url = "http://"+ipEstacion+"/CorpogasService/api/tickets/validaPosicionDisponible/estacionId/"+EstacionId+"/posicionCargaId/"+posicioncarga;
        // Utilizamos el metodo Post para validar la contraseña
        StringRequest eventoReq = new StringRequest(Request.Method.GET,url,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        try {
                            ...
                            String correcto = p1.getString("Correcto");
                            if (correcto.equals("true")){
                                finalizaventa(posicioncarga, usuario);
                            }else{
                                //Despacho en proceso
                                try {
                                    String mensaje = p1.getString("Mensaje");
